    ...
    <div class="eckb-category-archive-reset eckb-category-archive-defaults <?php esc_attr_e($preset_style); ?>">
        <header class="eckb-category-archive-header">
          ...
          if( ! is_null( $thisObj->term_id ) ){
            ...
            $separator = ' <span class="eckb-breadcrumb-link-icon ' . esc_html($kb_config['breadcrumb_icon_separator']) . '"></span>';
            $first_link = '<a href="' . EPKB_KB_Handler::get_first_kb_main_page_url( $kb_config ) . '">' . $kb_config['breadcrumb_home_text'] . '</a>';
            echo '<div class="eckb-category-breadcrumbs"><div class="eckb-breadcrumb-label">You are here:</div><ul class="eckb-breadcrumb-nav"><li>' . $first_link . $separator . '</li><li>' . get_taxonomy_parents('epkb_post_type_1_category',$currentCatID,true, $separator . '</li><li>') . '</li></ul></div>';
          }
          ?>
          <!-- /BREADCRUMBS -->
        </header>
